...
BEGIN
	...
	IF(@ID_RUBRO IS NULL) OR (@ID_RUBRO = 0)-- VALIDA EL INGRESO DE UN RUBRO OBLIGATORIO
	BEGIN
		SET @FLAG_ERROR = 1;
	...
	IF(@DIA_REND IS NULL) OR (@DIA_REND = 0)-- VALIDA EL INGRESO DE UN DIA DE RENDICION OBLIGATORIO
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar un dia de rendicion para el empresa a crear';
		RETURN
	END
	...
	BEGIN
		...
		INSERT INTO SUBSELECT.EMPRESA
		(ID_EMPRESA,NOMBRE,CUIT,DIRECCION,ID_RUBRO,DIA_REND)
		VALUES
			(NEXT VALUE FOR SUBSELECT.EMPRESA_SEQ ,@NOMBRE,@CUIT,@DIRECCION,@ID_RUBRO,@DIA_REND);
...
BEGIN 
	...
	SET  NOMBRE = @NOMBRE, CUIT = @CUIT,   
			DIRECCION=@DIRECCION,ID_RUBRO=@ID_RUBRO, DIA_REND= @DIA_REND
...
BEGIN
	...
	BEGIN
	 SELECT E.ID_EMPRESA, E.NOMBRE, E.CUIT, 
			E.DIRECCION, E.ID_RUBRO, R.DESCRIPCION,E.DIA_REND, E.ACTIVO 
...
BEGIN
	...
	BEGIN
		...
		SELECT @ID_SUCURSAL,U.ID_USUARIO FROM SUBSELECT.USUARIO U
	END
END
GO
...
BEGIN
	...
	END
	 ...
	 SELECT F.*
		, (case when F.NRO_FACTURA in (SELECT PF.NRO_FACTURA FROM SUBSELECT.PAGO_FACTURAS PF)
					then 1 else 0 end) as PAGADO
					, (case when F.NRO_FACTURA in (SELECT RF.NRO_FACTURA FROM SUBSELECT.REDENCION_FACTURA RF)
					then 1 else 0 end) as RENDIDO
		FROM SUBSELECT.FACTURA F
		WHERE F.ID_EMPRESA = @ID_EMPRESA
			AND  NOT EXISTS (SELECT 1 FROM SUBSELECT.PAGO_FACTURAS PF WHERE PF.NRO_FACTURA = F.NRO_FACTURA )
END
GO
...
CREATE PROCEDURE SUBSELECT.GET_FACTURAS_NO_PAGA
AS
BEGIN
	SELECT 
		F.* , (case when F.NRO_FACTURA in (SELECT PF.NRO_FACTURA FROM SUBSELECT.PAGO_FACTURAS PF)
					then 1 else 0 end) as PAGADO
					, (case when F.NRO_FACTURA in (SELECT RF.NRO_FACTURA FROM SUBSELECT.REDENCION_FACTURA RF)
					then 1 else 0 end) as RENDIDO
	FROM SUBSELECT.FACTURA F
...
BEGIN
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar un porcentaje que se cobra menor a 100';
		RETURN
	END
	...
	SELECT  F.NRO_FACTURA, F.TOTAL FROM SUBSELECT.FACTURA F, SUBSELECT.PAGO_FACTURAS PF, SUBSELECT.PAGO P
		...
		AND P.FECHA <= @FECHA_RENDICION AND P.FECHA >= dateadd(month, -1, @FECHA_RENDICION)
	...
	VALUES
		(NEXT VALUE FOR SUBSELECT.REDENCION_SEQ, @FECHA_RENDICION,@ID_EMPRESA,@ITEM
			,(SELECT SUM(TOTAL) FROM @FACTURAS_A_RENDIR),@PORCENTAJE,(SELECT SUM(TOTAL) FROM @FACTURAS_A_RENDIR) * (@PORCENTAJE/100)
...
BEGIN
	...
	DELETE FROM SUBSELECT.REDENCION
		WHERE  NRO_REDENCION = @NRO_RENDICION
...
BEGIN
	...
	WHERE E.ID_EMPRESA = R.ID_EMPRESA AND RF.NRO_REDENCION = R.NRO_REDENCION AND
...
BEGIN
	...
	SELECT  FC.NOMBRE,FC.APELLIDO, FC.DNI , (FC.CANTIDAD / FN.CANTIDAD * 100) TOTAL FROM @FACTURAS_COBRADAS FC , @FACTURAS_NO_COBRADAS FN
...
CREATE PROCEDURE SUBSELECT.SP_ABM_ITEMS_FACTURA_ELIMINAR
@ID_ITEM INT,
@FLAG_ERROR BIT OUTPUT,
@MENSAJE VARCHAR(255) OUTPUT
AS
BEGIN
	...
	DECLARE @MONTOARESTAR NUMERIC(18,2);
	
	/* CARGO FACTURA Y MONTO A RESTAR */
	SELECT 
		@MONTOARESTAR = MONTO, 
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'No se encontro el item a borrar'; 
	END
		
	/* RESTAMOS LA CANTIDAD ACTUAL A LA FACTURA */
	UPDATE
		SUBSELECT.FACTURA
	SET
		TOTAL = TOTAL - @MONTOARESTAR
	WHERE
		NRO_FACTURA = @NRO_FACTURA;

	/* POR ULTIMO BORRAMOS EL ITEM */
	DELETE FROM
		SUBSELECT.ITEM
	WHERE
		ID_ITEM = @ID_ITEM;
    ...
    SET @FLAG_ERROR = 0;
	SET @MENSAJE = 'Se borro el item correctamente';	
END


GO

CREATE PROCEDURE SUBSELECT.SP_ABM_ITEMS_FACTURA_MODIFICAR
@CANTIDAD NUMERIC(18,0),
@MONTO NUMERIC(18,2),
@ID_ITEM INT,
@FLAG_ERROR BIT OUTPUT,
...
BEGIN
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar la cantidad para el item';
		RETURN
	END
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar un monto para el item';
		RETURN
	END
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'No se encontro el item a modificar'; 
	END		

	/* RESTAMOS LA CANTIDAD ACTUAL Y SUMAMOS LA NUEVA */
	UPDATE
		SUBSELECT.FACTURA
	SET
		TOTAL = TOTAL - @MONTOARESTAR + @MONTO
	WHERE
		NRO_FACTURA = @NRO_FACTURA;
    ...
    SET @FLAG_ERROR = 0;
	SET @MENSAJE = 'Se modifico el item correctamente';	
END

GO

GO
CREATE PROCEDURE SUBSELECT.BORRAR_FACTURAS
@NRO_FACTURA NUMERIC(18,0),
@FLAG_ERROR BIT OUTPUT,
@MENSAJE VARCHAR(255) OUTPUT
AS
BEGIN
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'No se puede borrar una factura rendida.';
		RETURN
	END
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'No se puede borrar una factura pagada.';
		RETURN
	END

	DELETE FROM SUBSELECT.ITEM WHERE NRO_FACTURA = @NRO_FACTURA;

	DELETE FROM SUBSELECT.FACTURA WHERE NRO_FACTURA = @NRO_FACTURA;
	SET @FLAG_ERROR = 0;
	SET @MENSAJE = 'Se borro la factura';
