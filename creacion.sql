...
BEGIN
	...
	VALUES
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION = 'ABM DE CLIENTES')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION = 'ABM DE SUCURSAL')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION ='ABM DE EMPRESA')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION ='ABM DE ROL')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION ='LISTADO ESTADISTICO'));
	
	/*FUNCIONALIDADES DEL ROL AFILIADO*/
	SELECT @ID_ROL = ID_ROL FROM SUBSELECT.ROL WHERE DESCRIPCION='CLIENTES';
	INSERT INTO SUBSELECT.ROL_FUNCIONALIDAD (ID_ROL,ID_FUNCIONALIDAD)
	VALUES
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION ='CONSULTAR FACTURAS'));
	
	/*FUNCIONALIDADES DEL ROL PROFESIONAL*/
	SELECT @ID_ROL = ID_ROL FROM SUBSELECT.ROL WHERE DESCRIPCION='COBRADORES';
	INSERT INTO SUBSELECT.ROL_FUNCIONALIDAD (ID_ROL,ID_FUNCIONALIDAD)
	VALUES
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION ='REGISTRAR FACTURAS')),
	(@ID_ROL,(SELECT ID_FUNCIONALIDAD FROM SUBSELECT.FUNCIONALIDAD WHERE DESCRIPCION = 'CONSULTAR REDENCION'));
...
BEGIN
	...
	UPDATE @MIGRA_CLIENTES SET E_MAIL = E_MAIL +  convert(varchar(255),NRO_DOCUMENTO) WHERE E_MAIL IN(SELECT * FROM @MIGRA_CLIENTES_MAILS_REPETIDOS);
...
BEGIN
	...
	(NRO_PAGO,FECHA,ITEM_PAGO,TOTAL,FORMA_PAGO,ID_SUCURSAL, ID_CLIENTE)
	SELECT DISTINCT  (M.ItemPago_nro), M.Pago_Fecha, M.ItemPago_nro,M.Total, F.ID_FORMA_PAGO, S.ID_SUCURSAL, C.ID_CLIENTE
	FROM GD_ESQUEMA.MAESTRA M , SUBSELECT.SUCURSAL S, SUBSELECT.CLIENTE C, SUBSELECT.FORMA_PAGO F
	...
	(NRO_PAGO,FECHA,TOTAL,ID_FORMA_PAGO,ID_SUCURSAL,ITEM_PAGO,ID_CLIENTE)
	SELECT NRO_PAGO,FECHA,TOTAL,FORMA_PAGO,ID_SUCURSAL,ITEM_PAGO,ID_CLIENTE
	FROM @MIGRA_PAGO;
...
BEGIN
	...
	FROM SUBSELECT.USUARIO_ROL UR, SUBSELECT.USUARIO U, SUBSELECT.ROL R
...
BEGIN
	...
	FROM SUBSELECT.USUARIO_SUCURSAL US, SUBSELECT.USUARIO U, SUBSELECT.SUCURSAL S
	WHERE U.ID_USUARIO = @ID_USUARIO
...
BEGIN
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar un monto total  para la facturacion a crear';
		RETURN
	END
	...
	BEGIN
		SET @FLAG_ERROR = 1;
		SET @MENSAJE = 'Debe ingresar un monto total superior a 0 para la facturacion a crear';
		RETURN
	END
	...
	VALUES
		(NEXT VALUE FOR SUBSELECT.FACTURA_SEQ ,@ID_CLIENTE ,@ID_EMPRESA,@FECHA,@FECHA_VENCIMIENTO,@TOTAL);
END
...
BEGIN
	...
	INSERT INTO SUBSELECT.PAGO
	(NRO_PAGO,FECHA,TOTAL,ID_FORMA_PAGO,ID_SUCURSAL,ID_CLIENTE,ITEM_PAGO)	
	VALUES
		(NEXT VALUE FOR SUBSELECT.ITEM_PAGO_SEQ ,@FECHA,@TOTAL,@ID_FORMA_PAGO,@ID_SUCURSAL
				,@ID_CLIENTE, @ITEM_PAGO);
END
...
BEGIN
	...
	SET @ID_RENDICION = CONVERT (INT,(SELECT CURRENT_VALUE FROM SYS.SEQUENCES WHERE [NAME]='REDENCION_SEQ'));
...
BEGIN
	 ...
	 SELECT RUBRO.ID_RUBRO, RUBRO.DESCRIPCION
	 FROM SUBSELECT.RUBRO
